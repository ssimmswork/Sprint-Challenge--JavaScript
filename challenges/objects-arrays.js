...
const contactInfo = [];
let conInfo;
for (conInfo in graduates) {
    contactInfo.push((graduates[conInfo].first_name) + ' ' + (graduates[conInfo].email))
}
...
const uni = [];
let uniInfo;
const uniContent = "Uni";

for (uniInfo in universities) {
    if (universities[uniInfo].includes(uniContent)) {
        uni.push(universities[uniInfo]);
    }
}
console.log(uni);
...
The zoo wants to display both the scientific name and the 
animal name in front of the habitats.  Return an array with only the animal and scientific names in it.  
